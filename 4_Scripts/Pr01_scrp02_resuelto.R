...
# 3.2 Calcular los valores mínimo, promedio y máximo
Power.kW.MIN  <-min(subset$Power.kW.)
Power.kW.MEAN <-mean(subset$Power.kW.)
Power.kW.MAX  <-max(subset$Power.kW.)
...
for (i in 1:12)
{
  salida<-sumario_carga_mes(dataset,i)
  vect_MIN[i]  <- salida[1]
...
# 4.3 Plotear los valores
plot(1:12,vect_MAX, col="red",
     xlab="Mes", ylab="Carga térmica [kW]",
     main="Sumario de carga mensual")
points (1:12,vect_MEAN, col="green")
points (1:12,vect_MIN, col="blue")
